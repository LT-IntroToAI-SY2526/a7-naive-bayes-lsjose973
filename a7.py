...
class BayesClassifier:
    ...
    def classify(self, text: str) -> str:
        ...
        sum_pos_freqs = sum(self.pos_freqs.values())
        sum_neg_freqs = sum(self.neg_freqs.values())
        ...
        for token in tokens:
            ...
            times_in_pos += 1
            prob_pos += math.log(times_in_pos/sum_pos_freqs)
            times_in_neg = 0
            if token in self.neg_freqs:
                times_in_neg += self.neg_freqs[token]
            times_in_neg += 1
            prob_neg += math.log(times_in_neg/sum_neg_freqs)
